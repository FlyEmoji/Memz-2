...
const CGFloat kPageControlDotsOriginalWidth = 7.0f;
const CGFloat kPageControlDotsOriginalHeight = 7.0f;
const CGFloat kPageControlDotsOriginalSpace = 6.0f;
const CGFloat kUndefinedFloatValue = INFINITY;
...
@property (nonatomic, assign) CGSize sizeForDotSection;  // size for dots section (from the first to the last one)
@property (nonatomic, strong) NSMutableArray *dotsArray;  // array of dots sorted from left to right

@end

@implementation MZPageControl
@synthesize leftDotImageActive = _leftDotImageActive;
@synthesize rightDotImageActive = _rightDotImageActive;
@synthesize leftDotImageInactive = _leftDotImageInactive;
@synthesize rightDotImageInactive = _rightDotImageInactive;
...
- (void)forceUpdateDots:(BOOL)forced {
	...
	for (NSInteger i = 0; i < [self numberOfDotsToDisplay]; i++) {
		...
		// (4) Update dot if needed (if custom size has been set).
		if (dotSize.width != kUndefinedFloatValue) {
			UIImageView *view = [[UIImageView alloc] initWithFrame:CGRectMake([self xOriginForDotAtIndex:i],
																																				(self.frame.size.height - dotSize.height) / 2.0f,
																																				dotSize.width,
																																				dotSize.height)];
			view.opaque = NO;
			...
			UIView *view = [[UIView alloc] initWithFrame:CGRectMake([self xOriginForDotAtIndex:i],
																															(self.frame.size.height - kDefaultPageControlSize.height) / 2.0f,
																															kPageControlDotsOriginalWidth,
																															kPageControlDotsOriginalHeight)];
...
- (void)updateSizeForDotSection {
	self.sizeForDotSection = CGSizeMake([self totalWidthForNumberOfPages:self.numberOfPages],
																			[self maxHeightForNumberOfPages:self.numberOfPages]);
}
...
- (CGFloat)totalWidthForNumberOfPages:(NSInteger)numberOfPages {
	if (numberOfPages == 0) {
		...
		return [self leftDotSize].width + self.dotsSpace + [self middleDotsSectionWidthForNumberOfPages:numberOfPages]
			+ self.dotsSpace + [self rightDotSize].width;
	}
}
...
- (CGFloat)maxHeightForNumberOfPages:(NSInteger)numberOfPages {
	...
	if (numberOfPages == 0) {
		...
		maxHeight = fmax([self rightDotSize].height,
										 [self leftDotSize].height);
	} else {
		maxHeight = fmax(fmax([self rightDotSize].height,
													[self leftDotSize].height),
										 fmax([self middleDotSizeSelected:YES].height,
													[self middleDotSizeSelected:NO].height));
	}
	return maxHeight;
}
...
- (void)sizeToFit {
	self.frame = CGRectMake(self.frame.origin.x,
													self.frame.origin.y,
													[self sizeForNumberOfPages:self.numberOfPages].width,
													[self sizeForNumberOfPages:self.numberOfPages].height);
}
...
- (UIImage *)leftDotImageInactive {
	return _leftDotImageInactive ?: self.middleDotImageInactive;
}

- (UIImage *)leftDotImageActive {
	return _leftDotImageActive ?: self.middleDotImageActive;
}

- (UIImage *)rightDotImageInactive {
	return _rightDotImageInactive ?: self.middleDotImageInactive;
}

- (UIImage *)rightDotImageActive {
	return _rightDotImageActive ?: self.middleDotImageActive;
}
